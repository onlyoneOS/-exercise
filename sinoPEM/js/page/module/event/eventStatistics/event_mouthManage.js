	...
	function renderFusionCharts(searchType){
		...
		if(searchType=='room'){
			...
			surl=ctx+"/eventMouthManageInfo/getRoomEvent?startTime="+startTime;
			...
			surl=ctx+"/eventMouthManageInfo/getRoomDevType?startTime="+startTime;
	...
	function detailInfo(mid){
		...
		var id = mid;
		var url = ctx+"/eventMouthManageInfo/getDeviceName?roomName="+id+"&startTime="+startTime;
	...
	function devTypeDetailInfo(mid){
		...
		var  endTime= $('#endTime').val();
		var id = mid;
		var url = ctx+"/eventMouthManageInfo/getRoomName?devName="+id+"&startTime="+startTime;
		var murl = encodeURI(encodeURI(url));
		$('#edit_list').empty();
		$('#edit_list').load(murl,function(){
	...
	function export_Page() {
		var url="";
			if($('input:radio[name="searchType"]:checked').val()==0){
				 ...
				 url = ctx + "/eventMouthManageInfo/exportExcel?tmp="+ Math.random() + "&startTime="+ $('#startTime').val();
			}
			 if($('input:radio[name="searchType"]:checked').val()==1){
				
				 url = ctx + "/eventMouthManageInfo/exportExcel?tmp="+ Math.random() + "&startTime="+ $('#startTime').val()+"&exportId="+ $('#_exportId').val();
			}
	...
	function renderFusionTableCharts() {
		...
		*/
		$.ajax({
			...
			async : false,
			dataType : "json",
			url : ctx + "/eventMouthManageInfo/getEventHighCharts?tmp="
	...
	function renderFusionPieCharts() {
		...
		$.ajax({
			...
			async:false,
			dataType:"json",
			url:ctx+"/eventMouthManageInfo/getEventPieHighCharts?tmp="
