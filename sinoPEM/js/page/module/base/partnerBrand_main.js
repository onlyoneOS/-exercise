...
//				   }
		...
		table=dataTable$('#taskTable').dataTable({
			"bProcessing": true,
			"bServerSide": true,
			"sAjaxSource":ctx+"/base/partnerBrand/getList?tmp="+Math.random(), 
			"bRetrieve": true,
			"bSort": false,
			"bFilter": true,
			"sServerMethod": "POST",
			"aoColumns": [
						    { "mData": "brandName","mRender":function(data,row,obj){
							...
							{ "mData": "brandOid" },
							{ "mData": "brandCode" },
							{ "mData": "brandLevel" ,"mRender":function(data){
								var name = "";
								for(var i in brandLevelJson){
									if(data == brandLevelJson[i].id){
										name = brandLevelJson[i].name;
									}
								}
								return name;
							}},
							{ "mData": "brandLogo","mRender":function(data){
								if(data == null||data ==""){
									return "";
								} else {
									return '<img width="80px" height="30px;" src="'+ctx+data+'">';
								}
							}},
							{ "mData": "partnerName" }
						],
	...
	function _createOrEdit(id){
		...
		var options = {};
		options.murl = 'base/partnerBrand/partnerBrand_saveOrUpdate';
		options.keyName = 'id';
		options.keyValue = id;
		$.openurl(options);
	}
	function remove(id){
		$.messager.confirm('提示', '确定要删除这条信息吗?', function(r){
			if (r){
				$.ajax({
					url: ctx+"/base/partnerBrand/remove?id="+id,  // 提交的页面
