...
//	 require("formStaff");
  ...
  exports.init = function(){
			...
			$("document").ready(function(){
				 ...
				 $("#reset").unbind("click").click(function(){  		
					    $('#edit_list').empty();
				        $('#edit_list').load(ctx + '/counters/counterManagePage?tmp=' + Math.random());
                ...
                $("#add1").unbind('click').click(function(){
			    ...
			    //	  var flag = nameValidate();
			    	...
			    	//  if(modelFlag)  return;
					  ...
					  var url = ctx+"/counters/addCountersDev";		  	  
					  $("#addForm").ajaxSubmit({
						  url:url,
						  type:"post",
						  success : function(re){
							var pUrl = ctx + '/counters/counterManagePage?tmp=' + Math.random();
							$('#edit_list').empty();
				   		    $('#edit_list').load(pUrl);				   	   
						  }
					 });
  ...
  function  getOrgAndRoomTree(){
		...
		var optionsOrg = {
			...
			inputChange:function(){					
					var  orgSelectId = $("#roomId").formTree("getValue");
					$("#roomIdValue").attr("value", orgSelectId);
   ...
   */
    function   getProductType(){
    	...
    	//	inputValue : $('#productTypeName').val(),
    		...
    		animate : true,
    		searchTree : true,
    		tree_url : ctx+"/base/productType/getTreeResource?resourceId=010402&tmp="+Math.random(),// 顶层
   ...
   *  获取厂商信息
   */
  function getPartnerInfo(parternerUrl){
...
//						console.info(data);
	...
	function  getProductLine(url){
		...
		var lineTree = $("#_sino_product_line");
		var optionLineTree = {				
				inputName : '_sino_product_line',
			//	inputValue :$("#productLineName").val(),
				Checkbox : false,
				animate : true,
				searchTree : true,
				tree_url : url,// 顶层
		 ...
		 lineTree.resIds = $('#productLine').val();
		 lineTree.inputValue = $('#productLineName').val();			
		 lineTree.formTree(optionLineTree);		
	}
	
  /**
   *  获取型号信息
   */
	function getProductModel(modelUrl){
	...
	function   getOrgTree(){
		...
		var optionsOrg = {
			...
			inputChange:function(){					
				...
				//alert("orgId       "+orgSelectId);
				$("#orgValue").attr("value", orgSelectId);
