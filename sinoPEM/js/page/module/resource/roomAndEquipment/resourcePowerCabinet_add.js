	 ...
	 $(function() {
		...
		var optionsOrg = {
			...
			$("#productTypeDiv").formTree({
				...
				animate : true,
				searchTree : true,
				tree_url : ctx+"/base/productType/getTreeResource?resourceId=010401&tmp="+Math.random(),// 顶层
...
//	    			alert('devFormAjax');
    				$.ajax({
    					url: ctx+"/powerCabinet/addPowerDev",  // 提交的页面
...
//    							 $('#deviceName').val(data)
    							...
    							$('#alertMsg').empty();
    							$('#alertMsg').append('<div class="alert alert-success"><strong>错误</strong>配电柜添加失败,请您稍后再试!<button type="button" class="close" data-dismiss="alert">&times;</button></div>');
...
//	    			alert('devManagFormAjax');
    				$.ajax({
    					url: ctx+"/powerCabinet/addPowerDevManage",  // 提交的页面
