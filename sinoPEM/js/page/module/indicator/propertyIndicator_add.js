...
//		$formSelectProductModel.formSelect(optionProductModel);
		...
		$("#indicatorCategoryManage").unbind('click').click(function(){
			var categoryUrl = ctx+"/configIndicator/indicatorCategroyManage?urltype=menu&indicatorType=performance&tmp=" + Math.random();
		 ...
		 $("#dsave").unbind('click').click(function () {
	    	 $("input,select,textarea").not("[type=submit]").jqBootstrapValidation({
    		     submitSuccess: function (form, event) {
    		         ...
    		         $.ajax({
 		                url: ctx+"/propertyIndicator/savePropertyIndicator",  // 提交的页面
 		                ...
 		                error: function(request) {      // 设置表单提交出错
 		                	 $('#alertMsg').empty();
 			         			$('#alertMsg').append('<div class="alert alert-error"><strong>错误：</strong>性能指标添加失败，请稍后再试！<button type="button" class="close" data-dismiss="alert">&times;</button></div>');
 		                ...
 		                // 设置表单提交完成使用方法
 		                   if(data=="success"){
 		                	   $('.edit_list').load(ctx + '/propertyIndicator/propertyIndicatorManage?tmp=' + Math.random(),{},function(){
 		                			$('#alertMsg').empty();
 				         			$('#alertMsg').append('<div class="alert alert-success"><strong>提示：</strong>性能指标添加成功！<button type="button" class="close" data-dismiss="alert">&times;</button></div>');
 				         		    $(".alert").delay(2000).hide(0);
 				         		    $(".close").click(function(){
 				         		    	$(".alert").hide();
 				         		    });
