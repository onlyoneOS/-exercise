	...
	function load() {
		...
		var indicatorDataTypeId = $("#indicatorDataType").val();
		...
		var indicatorCategoryUrl=ctx+"/configIndicator/findIndicatorCategroy?tmp="+Math.random();
		...
		$(function() {
	    	 $("input,select,textarea").not("[type=submit]").jqBootstrapValidation({
    		     submitSuccess: function (form, event) {
    		         ...
    		         $.ajax({
 		                url: ctx+"/configIndicator/editConfigIndicator",  // 提交的页面
 		                ...
 		                error: function(request) {      // 设置表单提交出错
 		                	 $('#alertMsg').empty();
 			         			$('#alertMsg').append('<div class="alert alert-error"><strong>错误：</strong>配置指标修改失败，请稍后再试！<button type="button" class="close" data-dismiss="alert">&times;</button></div>');
 		                   ...
 		                   if(data=="success"){
 		                	  $('.edit_list').empty();
 		                	   $('.edit_list').load(ctx + '/configIndicator/configIndicatorManage?tmp=' + Math.random(),{},function(){
 		                			$('#alertMsg').empty();
 				         			$('#alertMsg').append('<div class="alert alert-success"><strong>提示：</strong>配置指标修改成功！<button type="button" class="close" data-dismiss="alert">&times;</button></div>');
 				         		    $(".alert").delay(2000).hide(0);
 				         		    $(".close").click(function(){
 				         		    	$(".alert").hide();
 				         		    });
